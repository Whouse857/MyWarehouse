...
// نسخه اصلاح شده: نمایش کد 12 رقمی اختصاصی در کارت‌ها

// تابع کمکی برای تولید کد بر اساس تنظیمات (دریافت شده از window یا props در صورت نیاز)
// در اینجا فرض بر این است که globalConfig از طریق API یا کانتکست در دسترس است.
// برای سادگی، یک تابع جنرال استفاده می‌کنیم.
...
const WithdrawPage = ({ user, serverStatus }) => {
    ...
    const [globalConfig, setGlobalConfig] = useState(null); // اضافه شدن استیت کانفیگ
    ...
    const filteredParts = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            res = res.filter(p => 
                ...
                (getPartCodeLocal(p, globalConfig).toLowerCase().includes(lower)) // جستجو بر اساس کد 12 رقمی
            );
        }

...
def init_db():
    try:
        with get_db_connection() as conn:
            ...
            # --- [SECURE INIT] ---
            # اگر ادمین وجود ندارد، بسازیمش. اگر هست کاری نداریم
            if not conn.execute("SELECT * FROM users WHERE username = 'admin'").fetchone():
                admin_perms = DEFAULT_PERMISSIONS_SCHEMA.copy()
                ...
                conn.execute("INSERT INTO users (username, password, role, full_name, permissions) VALUES (?, ?, ?, ?, ?)", 
                             ('admin', hash_password('admin'), 'admin', 'مدیر سیستم', json.dumps(admin_perms)))
                conn.commit() 
...
def fetch_daily_usd_price():
    global USD_CACHE
    if not SCRAPING_AVAILABLE: return 0
    today_str = datetime.now().strftime("%Y-%m-%d")
    if USD_CACHE["date_str"] == today_str and USD_CACHE["price"] > 0: return USD_CACHE["price"]
    try:
        ...
        if response.status_code == 200:
            ...
            if price_tag:
                ...
                return price
    except Exception as e: print(f"[WARNING] USD Scraper Error: {e}")
...
def list_backups() -> Response:
    try:
        ...
        for f in files:
            ...
            try: dt = datetime.strptime(f.replace("nexus_backup_", "").replace(".db", ""), "%Y-%m-%d_%H-%M-%S"); readable_date = dt.strftime("%Y-%m-%d %H:%M:%S")
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            if not user and username == 'admin':
                print("[WARN] Admin user missing in DB! Activating Self-Healing...")
                try:
                    admin_perms = DEFAULT_PERMISSIONS_SCHEMA.copy()
                    for k in admin_perms: admin_perms[k] = True
                    conn.execute("INSERT INTO users (username, password, role, full_name, permissions) VALUES (?, ?, ?, ?, ?)", 
                                 ('admin', hashed_input_password, 'admin', 'مدیر سیستم', json.dumps(admin_perms)))
                    conn.commit()
                    user = conn.execute("SELECT * FROM users WHERE username = ?", (username,)).fetchone()
                except Exception as e:
                    print(f"[CRITICAL ERROR] Failed to recreate admin: {e}")
            
            if not user:
                return jsonify({"success": False, "message": "نام کاربری یافت نشد"}), 401
                
            if user['password'] != hashed_input_password:
                return jsonify({"success": False, "message": "رمز عبور اشتباه است"}), 401
            
            # --- [ROBUST PERMISSION PARSING] ---
            # اینجا جایی است که برای اپراتورها حیاتی است
            raw_perms = user['permissions']
            final_perms = DEFAULT_PERMISSIONS_SCHEMA.copy()
            
            try:
                if raw_perms:
                    # تلاش اول: پارس کردن عادی
                    loaded_perms = json.loads(raw_perms)
                    
                    # تلاش دوم: اگر دوبار انکود شده بود
                    if isinstance(loaded_perms, str):
                        try: loaded_perms = json.loads(loaded_perms)
                        except: pass
                    
                    if isinstance(loaded_perms, dict):
                        # تمیزکاری داده‌ها: تبدیل "true" رشته‌ای به بولین
                        clean_perms = {}
                        for k, v in loaded_perms.items():
                            if str(v).lower() == 'true' or v == 1:
                                clean_perms[k] = True
                            elif str(v).lower() == 'false' or v == 0:
                                clean_perms[k] = False
                            else:
                                clean_perms[k] = v
                        final_perms.update(clean_perms)
            except Exception as e:
                print(f"[LOGIN PERMS WARNING] Failed to parse permissions for {username}: {e}")
                # در صورت خطا، دسترسی‌ها دیفالت (False) می‌مانند اما لاگین انجام می‌شود
...
def save_user() -> tuple:
    try:
        ...
        # تعیین نقش: اگر نام کاربری admin باشد، نقش هم admin است، وگرنه operator
        role = 'admin' if username == 'admin' else 'operator'
        full_name = d.get('full_name', '')
        mobile = d.get('mobile', '')
        
        received_perms = d.get('permissions', {})
        if not isinstance(received_perms, dict):
             received_perms = {}
        
        final_perms = DEFAULT_PERMISSIONS_SCHEMA.copy()
        final_perms.update(received_perms)
    ...
    try:
        ...
        with get_db_connection() as conn:
            user = conn.execute("SELECT id FROM users WHERE username = ? AND password = ?", 
                                (username, hash_password(old_pass))).fetchone()
            if not user:
                return jsonify({"success": False, "message": "رمز عبور فعلی اشتباه است"}), 401
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            if mode == 'category':
                ...
                config[new_val]['label'] = new_val # Simplified label update
        ...
        with get_db_connection() as conn:
            ...
            for row in rows:
                ...
                if q <= min_q:
                    ...
                    shortages.append({"val": row['val'], "pkg": row['package'], "qty": q, "min": min_q, "loc": row['storage_location'], "type": row['type'], "watt": row['watt'], "tolerance": row['tolerance'], "tech": row['tech'], "vendor": row['vendor_name'], "links": links})
            ...
            return jsonify({
                ...
                "total_value_usd_live": round(total_value_usd_live, 2), "live_usd_price": daily_usd_price, "shortages": shortages, "categories": categories
            })
    except Exception as e: return jsonify({"error": str(e)}), 500

if __name__ == '__main__':
    threading.Thread(target=watchdog_service, daemon=True).start()
    def open_browser(): time.sleep(1.5); webbrowser.open_new(SERVER_URL)

...
// نسخه بهینه‌شده: استفاده از useMemo برای جلوگیری از رندر اضافی در تشخیص تکراری‌ها
...
const EntryPage = ({ setView, serverStatus, user, globalConfig }) => {
    ...
    // --- Optimization: استفاده از useMemo به جای useEffect برای محاسبه تکراری‌ها ---
    const duplicates = useMemo(() => {
        ...
        return partsList.filter(p => {
            if (formData.id && p.id === formData.id) return false; // خود آیتم در حال ویرایش را نادیده بگیر
                                ...
                                <div className="space-y-4 pl-1 pr-1">
                                    <div className="flex flex-col"><label className="text-nexus-accent text-xs mb-1 block font-bold">دسته‌بندی قطعه (Component Type)</label>
                                        <div className="relative">
                                            {/* تغییر: مرتب‌سازی گزینه‌ها بر اساس اولویت */}

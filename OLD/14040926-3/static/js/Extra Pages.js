...
// نسخه نهایی: پیام کپی کوچک کنار موس (Tooltip) + بازطراحی کامل
...
// --- Contacts Page (Redesigned & Validated & Tooltip Feedback) ---
const ContactsPage = ({ serverStatus }) => {
    ...
    // استیت برای نمایش پیام کپی کنار موس
    ...
    useLucide([contacts, newContact.id, errors]);

    // تابع کپی با فیدبک کنار موس
    const copyToClipboard = (e, text) => {
        if (!text) return;
        e.stopPropagation(); // جلوگیری از کلیک شدن کارت
        navigator.clipboard.writeText(text);
        
        // نمایش فیدبک در موقعیت موس
        setCopyFeedback({ x: e.clientX, y: e.clientY });

        // مخفی کردن بعد از 1.5 ثانیه
        setTimeout(() => {
    ...
    const handleSave = async () => { 
        ...
        if (!hasPhone && !hasMobile) {
            ...
            isValid = false;
            notify.show('خطا', "حداقل یک شماره تماس (موبایل یا تلفن) الزامی است", 'error');
        } else {
            if (hasPhone && !validatePhoneFormat(newContact.phone)) { newErrors.phone = true; isValid = false; notify.show('خطا', "فرمت تلفن اشتباه است (شروع با 0، حداکثر 11 رقم)", 'error'); }
            if (hasMobile && !validatePhoneFormat(newContact.mobile)) { newErrors.mobile = true; isValid = false; notify.show('خطا', "فرمت موبایل اشتباه است (شروع با 0، حداکثر 11 رقم)", 'error'); }
        }
        ...
        try { 
            ...
            if (ok) { 
                ...
                setErrors({});
                notify.show('موفقیت', "اطلاعات با موفقیت ذخیره شد.", 'success'); 
            } 
    ...
    const preventNonNumeric = (e) => {
        if (!/[0-9]/.test(e.key) && e.key !== "Backspace" && e.key !== "Delete" && e.key !== "ArrowLeft" && e.key !== "ArrowRight" && e.key !== "Tab") { e.preventDefault(); }
    };
    ...
    return (
        <div className="flex-1 p-6 overflow-hidden flex flex-col h-full relative">
            ...
            {copyFeedback && (
                <div 
                    className="fixed z-[9999] pointer-events-none bg-nexus-primary text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg border border-white/20 animate-in fade-in zoom-in duration-200 flex items-center gap-1"
                    style={{ top: copyFeedback.y + 15, left: copyFeedback.x + 15 }}
                >
            ...
            <header className="h-16 shrink-0 border-b border-white/5 flex items-center justify-between mb-4">
                <div>
                    <h2 className="text-2xl font-black text-white flex items-center gap-3">
                        <i data-lucide="contact-2" className="text-nexus-primary w-6 h-6"></i>
                        مدیریت تامین‌کنندگان
                    </h2>
                    <p className="text-gray-400 text-xs mt-1">لیست دفترچه تلفن و آدرس‌های خرید</p>
                </div>
                <div className="w-64 relative">
                    <i data-lucide="search" className="absolute left-3 top-2.5 w-4 h-4 text-gray-500"></i>
                    <input 
                        className="nexus-input w-full pl-9 pr-3 py-2 text-sm rounded-xl bg-black/20 focus:bg-black/40 border-white/10" 
            ...
            <div className="flex-1 grid grid-cols-1 xl:grid-cols-12 gap-6 min-h-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {filteredContacts.length === 0 && (
                            <div className="col-span-full flex flex-col items-center justify-center py-20 text-gray-500 bg-white/5 rounded-3xl border border-dashed border-white/5">
                                <i data-lucide="user-x" className="w-12 h-12 mb-3 opacity-50"></i>
                                ...
                                <div key={c.id} className={`glass-panel p-5 rounded-3xl border transition-all duration-300 group relative overflow-hidden flex flex-col gap-3 hover:-translate-y-1 hover:shadow-xl ${isEditing ? 'border-nexus-primary bg-nexus-primary/5 ring-1 ring-nexus-primary/30' : 'border-white/5 hover:border-white/20'}`}>
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center text-white font-bold border border-white/10 shadow-inner">
                                                {c.name.charAt(0).toUpperCase()}
                                            </div>
                                            <div>
                                                <h3 className="font-bold text-white text-base truncate max-w-[150px]" title={c.name}>{c.name}</h3>
                                                {c.website && <a href={c.website} target="_blank" className="text-[10px] text-blue-400 hover:underline flex items-center gap-1"><i data-lucide="globe" className="w-3 h-3"></i> وب‌سایت</a>}
                                            </div>
                                        </div>
                                        <div className="flex gap-1">
                                            <button onClick={() => handleEdit(c)} className={`p-2 rounded-xl transition-colors ${isEditing ? 'bg-nexus-primary text-white shadow-lg' : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10'}`}><i data-lucide="pencil" className="w-4 h-4"></i></button>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-xs">
                                        {c.mobile && (
                                            <div onClick={(e) => copyToClipboard(e, c.mobile)} className="bg-black/20 p-2 rounded-xl border border-white/5 flex items-center gap-2 cursor-pointer hover:bg-black/40 transition group/item hover:border-nexus-primary/30">
                                                <div className="w-6 h-6 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-400"><i data-lucide="smartphone" className="w-3 h-3"></i></div>
                                                <span className="font-mono text-gray-300 group-hover/item:text-white transition-colors">{c.mobile}</span>
                                            </div>
                                        )}
                                        {c.phone && (
                                            <div onClick={(e) => copyToClipboard(e, c.phone)} className="bg-black/20 p-2 rounded-xl border border-white/5 flex items-center gap-2 cursor-pointer hover:bg-black/40 transition group/item hover:border-nexus-primary/30">
                                                <div className="w-6 h-6 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-400"><i data-lucide="phone" className="w-3 h-3"></i></div>
                                                <span className="font-mono text-gray-300 group-hover/item:text-white transition-colors">{c.phone}</span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        {c.email && (
                                            <div onClick={(e) => copyToClipboard(e, c.email)} className="flex items-center gap-2 text-xs text-gray-400 hover:text-white cursor-pointer transition p-1 rounded hover:bg-white/5 hover:text-nexus-primary">
                                                <i data-lucide="mail" className="w-3.5 h-3.5 text-orange-400"></i>
                                                <span className="font-mono truncate">{c.email}</span>
                                            </div>
                                        )}
                                        <div className="flex items-start gap-2 text-xs text-gray-400 bg-white/5 p-2 rounded-xl">
                                            <i data-lucide="map-pin" className="w-3.5 h-3.5 text-rose-400 mt-0.5 shrink-0"></i>
                                    ...
                                    {c.notes && (
                                        <div className="mt-auto pt-2 border-t border-dashed border-white/10">
                                            <p className="text-[10px] text-gray-500 italic truncate" title={c.notes}>{c.notes}</p>
                                        </div>
                                    )}
                                </div>
                            );
                ...
                <div className="xl:col-span-4 h-full overflow-y-auto custom-scroll">
                    <div className="glass-panel border border-white/10 rounded-2xl p-5 shadow-2xl relative bg-gradient-to-b from-white/5 to-[#020617]">
                        <div className="flex justify-between items-center mb-6 pb-4 border-b border-white/5 sticky top-0 bg-[#0f172a]/95 backdrop-blur z-10 -mx-5 px-5 -mt-5 pt-5 rounded-t-2xl">
                            <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                {newContact.id ? (
                                    <><i data-lucide="user-cog" className="text-nexus-accent w-5 h-5"></i> <span className="text-orange-100">ویرایش مخاطب</span></>
                                ) : (
                                    <><i data-lucide="user-plus" className="text-emerald-400 w-5 h-5"></i> <span className="text-emerald-100">افزودن مخاطب</span></>
                                )}
                            </h3>
                            {newContact.id && (
                                <button 
                                    ...
                                    className="px-2 py-1 rounded bg-white/5 hover:bg-white/10 text-xs font-bold text-gray-400 hover:text-white transition"
                                >
                                    انصراف
                                </button>
                            )}
                        ...
                        <div className="space-y-4">
                            <div className="bg-black/20 p-3 rounded-xl border border-white/5">
                                <NexusInput 
                                    ...
                                    error={errors.name}
                                    placeholder="مثلا: فروشگاه قطعات الکترونیک..."
                                />
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <NexusInput label="فکس" dir="ltr" value={newContact.fax} onChange={e => handleChange('fax', e.target.value)} disabled={!serverStatus} />
                                <NexusInput label="وب‌سایت" dir="ltr" value={newContact.website} onChange={e => handleChange('website', e.target.value)} disabled={!serverStatus} placeholder="example.com" />
                            </div>
                            ...
                            <div className="flex flex-col">
                                <label className="text-xs mb-1 block font-medium text-gray-400">توضیحات تکمیلی</label>
                                <textarea 
                                    className="nexus-input w-full px-3 py-2 text-sm min-h-[80px] resize-none bg-black/20 border-white/10 rounded-lg focus:border-nexus-primary/50" 
                                    value={newContact.notes} 
                                    onChange={e => handleChange('notes', e.target.value)} 
                                    disabled={!serverStatus}
                                    placeholder="یادداشت‌های اضافی درباره نحوه خرید، ساعت کاری و..."
                                />
                            </div>
                            ...
                            <button 
                                onClick={handleSave} 
                                disabled={!serverStatus} 
                                className={`w-full py-4 rounded-xl font-black text-white text-sm shadow-xl transition-all transform active:scale-95 flex items-center justify-center gap-2 ${newContact.id ? 'bg-gradient-to-r from-nexus-accent to-blue-600 shadow-blue-900/20' : 'bg-gradient-to-r from-nexus-primary to-indigo-600 shadow-indigo-900/20'}`}
                            >
                                <i data-lucide={newContact.id ? "save" : "plus-circle"} className="w-5 h-5"></i>
                                {newContact.id ? 'ذخیره تغییرات' : 'ثبت مخاطب جدید'}
                            </button>
                        </div>
                    </div>
                </div>
...
const LogPage = () => {
    ...
    useEffect(() => { 
        fetchAPI('/log').then(({ok, data}) => { if (ok) setLogList(data); setLoading(false); }).catch(console.error); 
    }, []);
    ...
    const filteredLogs = useMemo(() => {
        return logList.filter(l => {
            ...
            const searchableText = toEnglishDigits(`${l.val || ''} ${l.username || ''} ${l.operation_type || ''} ${l.reason || ''} ${l.vendor_name || ''} ${l.log_id || ''} ${logShamsi}`.toLowerCase());
    ...
    useLucide([filteredLogs, filters]); 
        ...
        <div className="flex-1 p-6 h-full flex flex-col">
            <header className="mb-6 flex flex-col gap-4">
                ...
                <div className="bg-white/5 p-3 rounded-xl border border-white/5 grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
                    <div className="md:col-span-3"><input className="nexus-input w-full px-3 py-2 text-sm bg-black/20 border-white/10 focus:bg-black/40" placeholder="جستجو (نام، دلیل، تاریخ)..." value={filters.general} onChange={e => setFilters({...filters, general: e.target.value})} /></div>
            ...
            <div className="flex-1 overflow-y-auto custom-scroll pr-2 relative">
                ...
                <div className="space-y-6 relative z-10 pb-10">
                    {filteredLogs.length === 0 ? (<div className="text-center text-gray-500 py-20 bg-white/5 rounded-2xl border border-dashed border-white/10"><i data-lucide="search-x" className="w-12 h-12 mx-auto mb-3 opacity-50"></i><p>موردی با این مشخصات یافت نشد.</p></div>) : (
                        filteredLogs.map(l => {
                                ...
                                <div key={l.log_id} className="flex gap-4 group animate-in slide-in-from-right-2 duration-300">
                                    ...
                                    <div className={`flex-1 glass-panel p-4 rounded-2xl border transition-all hover:bg-white/[0.02] ${isEdit ? 'border-yellow-500/20' : 'border-white/5'}`}>
                                        <div className="flex justify-between items-start mb-3">
                                            <div><div className="flex items-center gap-2 mb-1"><span className={`text-sm font-bold ${colorClass.split(' ')[0]}`}>{titleText}</span><span className="text-[10px] text-gray-500 px-2 py-0.5 bg-white/5 rounded-full font-mono">ID: {l.log_id}</span><span className="text-[10px] text-gray-400 bg-white/5 px-2 py-0.5 rounded-full">{l.operation_type}</span></div><h3 className="text-lg font-black text-white ltr font-mono text-right flex items-center gap-2">{l.val}<span className="text-xs font-normal text-gray-400 bg-white/5 px-2 py-1 rounded">{l.package || '-'}</span></h3></div>
                                            <div className="text-left flex flex-col items-end"><span className="text-xs text-gray-300 font-mono bg-white/5 px-2 py-1 rounded">{toShamsiDateTime(l.timestamp)}</span><span className="text-[10px] text-nexus-primary mt-1 flex items-center gap-1"><i data-lucide="user" className="w-3 h-3"></i> {l.username || 'سیستم'}</span></div>
                                        </div>

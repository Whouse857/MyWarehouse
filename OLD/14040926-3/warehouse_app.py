...
# --- [TAG: PERMISSION_HELPER] ---
# تابع حیاتی برای تمیزکاری داده‌های دسترسی
def parse_permissions_recursive(data, depth=0):
    """
    تضمین می‌کند که خروجی حتماً یک دیکشنری (آبجکت) باشد.
    رشته‌های تو در تو (JSON inside JSON string) را باز می‌کند.
    """
    if depth > 3: # جلوگیری از حلقه بی‌نهایت
        return {}
    ...
    if isinstance(data, str):
        ...
        try:
            ...
            # اگر خروجی پارس شده باز هم رشته بود، دوباره تلاش کن
            ...
            print(f"[WARN] Failed to parse permissions JSON: {data}")
            return {}
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            user = conn.execute("SELECT * FROM users WHERE username = ? AND password = ?", 
                (username, hashed_input_password)).fetchone()
            
            if user:
                # --- FIX: ROBUST PERMISSION PARSING ---
                # استفاده از تابع بازگشتی برای اطمینان از اینکه خروجی حتما دیکشنری است
                perms = parse_permissions_recursive(user['permissions'])
                
                print(f"[LOGIN] User: {username}, Role: {user['role']}, Parsed Perms: {perms}")
                
                return jsonify({
                    "success": True, 
                    "role": user['role'], 
                    "username": user['username'],
                    "permissions": perms,  # This is now guaranteed to be a DICT
...
def get_users() -> Response:
    with get_db_connection() as conn:
        ...
        for u in users:
            d = dict(u)
            # --- FIX: ROBUST PERMISSION PARSING FOR LIST ---
...
def save_user() -> tuple:
    try:
        ...
        # Ensure permissions are always saved as a clean JSON string
        perms_input = d.get('permissions', {})
        if isinstance(perms_input, str):
            # If client somehow sent a string, try to parse it first to normalize
            perms_input = parse_permissions_recursive(perms_input)

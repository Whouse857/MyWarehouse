# --- [TAG: IMPORTS] ---
# وارد کردن کتابخانه‌های استاندارد و ضروری
import logging
import os
import sqlite3
import threading
...
# تنظیمات اولیه و ثابت‌های مهندسی
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            if part_id:
                ...
                conn.execute("""UPDATE resistors SET val=?, watt=?, tolerance=?, package=?, type=?, buy_date=?, quantity=?, toman_price=?, reason=?, min_quantity=?, vendor_name=?, last_modified_by=?, storage_location=?, tech=?, usd_rate=? WHERE id=?""", (*payload.values(), part_id))
                rid = part_id
            else:
                ...
                if existing:
                    ...
                    conn.execute("UPDATE resistors SET quantity=?, toman_price=?, buy_date=?, vendor_name=?, last_modified_by=?, reason=?, usd_rate=? WHERE id=?", 
                                 (new_qty, payload['toman_price'], payload['buy_date'], payload['vendor_name'], username, payload['reason'], payload['usd_rate'], rid))
                else:
                    cur = conn.execute("INSERT INTO resistors (val, watt, tolerance, package, type, buy_date, quantity, toman_price, reason, min_quantity, vendor_name, last_modified_by, storage_location, tech, usd_rate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", tuple(payload.values()))
                    rid = cur.lastrowid
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            for item in items:
                ...
                row = conn.execute("SELECT quantity, val, toman_price, vendor_name FROM resistors WHERE id = ?", (part_id,)).fetchone()
                ...
                conn.execute("""
                    INSERT INTO purchase_log (resistor_id, val, quantity_added, unit_price, vendor_name, purchase_date, reason, operation_type, username) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
def delete_part(id: int) -> tuple:
    with get_db_connection() as conn:
        try:
            ...
            if part: conn.execute("INSERT INTO purchase_log (resistor_id, val, quantity_added, operation_type, reason) VALUES (?, ?, 0, 'DELETE', 'Deleted by user')", (id, part['val']))
        except: pass
...
# این بخش برای محاسبه آمارهای انبار اضافه شده است
@app.route('/api/inventory/stats', methods=['GET'])
def get_inventory_stats() -> Response:
    try:
        with get_db_connection() as conn:
            rows = conn.execute("SELECT val, quantity, toman_price, usd_rate, min_quantity, type, package, storage_location FROM resistors").fetchall()
            
            total_items = len(rows)
            total_quantity = 0
            total_value_toman = 0.0
            total_value_usd = 0.0
            shortages = []
            categories = {}
            ...
            for row in rows:
                ...
                total_quantity += q
                val_toman = q * p
                total_value_toman += val_toman
                
                # محاسبه ارزش دلاری فقط اگر نرخ ثبت شده باشد
                if u > 0:
                    total_value_usd += val_toman / u
                ...
                categories[cat]["value"] += val_toman
            
            # مرتب‌سازی کسری‌ها بر اساس بحرانی بودن (کمترین تعداد اول)
            shortages.sort(key=lambda x: x['qty'])
            
            return jsonify({
                "total_items": total_items,
                "total_quantity": total_quantity,
                "total_value_toman": total_value_toman,

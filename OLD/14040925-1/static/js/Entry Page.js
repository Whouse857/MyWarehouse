...
const SummaryModal = ({ isOpen, onClose, onConfirm, data }) => {
...
const EntryPage = ({ setView, serverStatus, user, globalConfig }) => {
    const [formData, setFormData] = useState({ id: null, val: "", unit: "", watt: "", tol: "", pkg: "", type: "Resistor", date: getJalaliDate(), qty: "", price_toman: "", usd_rate: "", reason: "", min_qty: 1, vendor_name: "", location: "", tech: "" });
    const [partsList, setPartsList] = useState([]);
    const [contacts, setContacts] = useState([]);
    const [filters, setFilters] = useState({ val: '', pkg: '', loc: '', type: '' });
    ...
    useEffect(() => {
        ...
        const found = partsList.filter(p => {
    ...
    }, [partsList, filters, formData.type]);
    ...
    useLucide([filteredParts.length, formData.type, duplicates.length]); 

    const handleChange = useCallback((key, val) => {
        let v = val;
        if (key === 'price_toman' || key === 'usd_rate') v = formatNumberWithCommas(val.replace(/,/g, ''));
    ...
    const handleFinalSubmit = async () => {
        ...
        try { 
            ...
            if (ok) { 
                ...
                setFormData({ 
                    id: null, val: "", unit: defUnit, watt: "", tol: "", pkg: "", type: formData.type, date: getJalaliDate(), qty: "", price_toman: "", usd_rate: "", reason: "", min_qty: 1, vendor_name: "", location: "", tech: "" 
                });
    ...
    const handleEdit = (p) => {
        ...
        setFormData({ 
            ...p, val: v, unit: u, tol: p.tolerance, pkg: p.package, type: category, tech: p.tech || "", watt: p.watt, date: p.buy_date, qty: (p.quantity === null || p.quantity === undefined) ? "" : p.quantity, price_toman: formatNumberWithCommas(p.toman_price), usd_rate: formatNumberWithCommas(p.usd_rate || ""), min_qty: (p.min_quantity === null || p.min_quantity === undefined) ? "" : p.min_quantity, location: p.storage_location || "" 
        });
        setErrors({});
    };
    ...
    const currentConfig = (globalConfig?.[formData.type]) ? globalConfig[formData.type] : (globalConfig?.["Resistor"]) || { units: [], paramOptions: [], packages: [], techs: [], icon: 'circle', label: 'Unknown' };

    return (
        <ErrorBoundary>
            <div className="flex-1 flex flex-col min-w-0 h-full relative">
                <SummaryModal isOpen={showSummary} onClose={() => setShowSummary(false)} onConfirm={handleFinalSubmit} data={formData} />
                    ...
                    <div className="grid grid-cols-12 gap-6">
                        <div className="col-span-12 lg:col-span-8 flex flex-col">
                            <div className="glass-panel rounded-2xl flex flex-col">
                                <div className="p-3 border-b border-white/5 grid grid-cols-4 gap-3 bg-white/5">
                        ...
                        <div className="col-span-12 lg:col-span-4 h-full">
                            <div className="glass-panel border-white/10 rounded-2xl p-5 h-full flex flex-col shadow-2xl relative">
                                ...
                                <div className="space-y-4 pl-1 pr-1">
                                    <div className="flex flex-col"><label className="text-nexus-accent text-xs mb-1 block font-bold">دسته‌بندی قطعه (Component Type)</label><div className="relative"><select value={formData.type} onChange={e=>handleChange('type', e.target.value)} disabled={!serverStatus} className="nexus-input w-full px-3 py-2 text-sm appearance-none cursor-pointer font-bold text-yellow-400 bg-slate-900/80 border-nexus-accent/30">{Object.keys(globalConfig).filter(key => key !== 'General').map(key => <option key={key} value={key}>{globalConfig[key].label}</option>)}</select></div></div>

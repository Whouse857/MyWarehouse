...
const App = () => {
    ...
    // 1. چک کردن مداوم اتصال به سرور (Heartbeat)
    useEffect(() => { 
    ...
    // 2. دریافت تنظیمات اولیه هنگام شروع
    useEffect(() => {
    ...
    // 3. اطلاع به سرور هنگام بستن مرورگر (برای بستن پروسه پایتون)
    useEffect(() => {
        const handleUnload = () => { 
    ...
    // 4. رفرش کردن آیکون‌های Lucide هنگام تغییر تب
    useEffect(() => { 
            ...
            <DialogContext.Provider value={dialogValue}>
                ...
                <div className="flex min-h-screen text-gray-300 font-sans">
                    ...
                    <aside className="w-64 flex flex-col border-l border-white/5 bg-[#020617]/80 backdrop-blur-xl shrink-0 z-50 sticky top-0 h-screen">
                        {/* هدر سایدبار */}
                        <div className="h-20 flex flex-col items-center justify-center border-b border-white/5 gap-1">
                    ...
                    <main className="flex-1 flex flex-col min-w-0 relative bg-nexus-bg">
                        {/* رندر کردن کامپوننت صفحه بر اساس تب فعال */}
                        
                        {/* صفحات عمومی */}
                        {activeTab === 'dashboard' && <DashboardPage setView={setActiveTab} role={user.role} />}
                        {activeTab === 'entry' && <EntryPage setView={setActiveTab} serverStatus={serverStatus} user={user} globalConfig={globalConfig} />}
                        {activeTab === 'withdraw' && <WithdrawPage user={user} serverStatus={serverStatus} />}
                        
                        {/* تغییر مهم: نمایش کامپوننت واقعی آمار به جای Placeholder */}
                        {activeTab === 'inventory' && <InventoryPage />} 
                        
                        {/* صفحات ادمین (فقط اگر ادمین باشد) */}
                        {activeTab === 'users' && user.role === 'admin' && <UsersPage serverStatus={serverStatus} />}
                        {activeTab === 'management' && user.role === 'admin' && <ManagementPage globalConfig={globalConfig} onConfigUpdate={setGlobalConfig} />}
                        {activeTab === 'backup' && user.role === 'admin' && <BackupPage />}

...
const ExitDialog = ({ isOpen, onClose, onConfirm, onBackupAndExit, title, message }) => {
    if (!isOpen) return null;
    return (
        <ModalOverlay>
            <div className="glass-panel border border-nexus-primary/50 p-6 rounded-2xl max-w-sm w-full shadow-2xl animate-scale-in" onClick={e => e.stopPropagation()}>
                <div className="flex flex-col items-center text-center gap-4">
                    ...
                    <div className="flex flex-col gap-2 w-full mt-4">
                        <button onClick={onBackupAndExit} className="w-full py-3 rounded-xl bg-nexus-success hover:bg-emerald-600 text-white font-bold transition shadow-lg flex justify-center items-center gap-2"><i data-lucide="save" className="w-4 h-4"></i>بک‌آپ گیری و خروج</button>
                        <div className="flex gap-2">
                            <button onClick={onClose} className="flex-1 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-gray-300 font-bold transition border border-white/5">انصراف</button>
                            <button onClick={onConfirm} className="flex-1 py-2.5 rounded-xl bg-nexus-danger hover:bg-red-600 text-white font-bold transition shadow-lg">فقط خروج</button>
                        </div>
                    </div>
                </div>
            </div>

...
const UsersPage = ({ serverStatus }) => {
    ...
    const handleEditUser = (user) => { 
        ...
        if (typeof userPerms !== 'object' || userPerms === null) userPerms = defaultPerms;
                                ...
                                <div className="flex flex-col gap-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={`text-[10px] px-2 py-0.5 rounded border ${
                                            u.permissions === 'admin' ? 'bg-purple-500/10 text-purple-400 border-purple-500/20' : 
                                            u.permissions === 'edit' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                                            u.permissions === 'view_only' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                                            'bg-red-500/10 text-red-400 border-red-500/20'
                                        }`}>
                                            {u.permissions === 'admin' ? 'مدیر سیستم' : 
                                             u.permissions === 'edit' ? 'اپراتور ارشد (اصلاح)' : 
                                             u.permissions === 'view_only' ? 'اپراتور ساده (مشاهده)' : 'مسدود'}
                                        </span>
                                    </div>
                ...
                <div className="glass-panel rounded-2xl p-6 h-fit sticky top-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-white/5 p-3 rounded-xl border border-white/10">
                            <label className="text-nexus-accent text-xs mb-3 block font-bold border-b border-white/10 pb-2">سطح دسترسی (مجوزها)</label>
                            
                            <div className="grid grid-cols-2 gap-2">
                                <label className="flex items-center gap-2 cursor-pointer group p-1 hover:bg-white/5 rounded">
                                    <input type="checkbox" className="accent-nexus-primary" checked={newUser.permissions?.entry} onChange={() => togglePermission('entry')} />
                                    <span className="text-xs text-gray-300">ورود قطعه</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer group p-1 hover:bg-white/5 rounded">
                                    <input type="checkbox" className="accent-nexus-primary" checked={newUser.permissions?.withdraw} onChange={() => togglePermission('withdraw')} />
                                    <span className="text-xs text-gray-300">برداشت قطعه</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer group p-1 hover:bg-white/5 rounded">
                                    <input type="checkbox" className="accent-nexus-primary" checked={newUser.permissions?.inventory} onChange={() => togglePermission('inventory')} />
                                    <span className="text-xs text-gray-300">آمار و موجودی</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer group p-1 hover:bg-white/5 rounded">
                                    <input type="checkbox" className="accent-nexus-primary" checked={newUser.permissions?.contacts} onChange={() => togglePermission('contacts')} />
                                    <span className="text-xs text-gray-300">تامین‌کنندگان</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer group p-1 hover:bg-white/5 rounded">
                                    <input type="checkbox" className="accent-nexus-primary" checked={newUser.permissions?.log} onChange={() => togglePermission('log')} />
                                    <span className="text-xs text-gray-300">تاریخچه (Log)</span>
                                </label>
                            </div>
                            
                            <div className="mt-3 pt-2 border-t border-white/10 grid grid-cols-1 gap-2">
                                <label className="flex items-center gap-2 cursor-pointer group p-1 hover:bg-red-500/10 rounded">
                                    <input type="checkbox" className="accent-red-500" checked={newUser.permissions?.users} onChange={() => togglePermission('users')} />
                                    <span className="text-xs text-red-300">مدیریت کاربران (Admin)</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer group p-1 hover:bg-red-500/10 rounded">
                                    <input type="checkbox" className="accent-red-500" checked={newUser.permissions?.management} onChange={() => togglePermission('management')} />
                                    <span className="text-xs text-red-300">تنظیمات لیست‌ها (Admin)</span>
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer group p-1 hover:bg-red-500/10 rounded">
                                    <input type="checkbox" className="accent-red-500" checked={newUser.permissions?.backup} onChange={() => togglePermission('backup')} />
                                    <span className="text-xs text-red-300">پشتیبان‌گیری (Admin)</span>
                                </label>
                            </div>
                        </div>
...
// --- Management Page (Sortable + Delete Category) ---
const ManagementPage = ({ globalConfig, onConfigUpdate }) => {
    ...
    const notify = useNotify();
    const dialog = useDialog();
    useLucide([selectedType, config, sortedKeys]); // Updated dependencies
    ...
    const handleDeleteCategory = async (keyToDelete) => {
        if (keyToDelete === 'General') {
            notify.show('خطا', 'حذف دسته تنظیمات عمومی مجاز نیست.', 'error');
            return;
        }

        const confirmed = await dialog.ask("حذف دسته", `آیا از حذف دسته‌بندی "${config[keyToDelete].label}" اطمینان دارید؟\nتوجه: این کار باعث حذف قطعات داخل انبار نمی‌شود، اما این دسته از لیست‌ها حذف خواهد شد.`, "danger");
        ...
        if (confirmed) {
            try {
                ...
                // ارسال تغییرات به سرور (ذخیره کانفیگ جدید)
                const { ok } = await fetchAPI('/settings/config', { method: 'POST', body: newConfig });
                
                if (ok) {
                    // آپدیت استیت‌ها
                    setConfig(newConfig);
                    const newSorted = sortedKeys.filter(k => k !== keyToDelete);
                    setSortedKeys(newSorted);
                    
                    // اگر دسته حذف شده در حال نمایش بود، سوئیچ کن به اولی
                    if (selectedType === keyToDelete) {
                        setSelectedType(newSorted.length > 0 ? newSorted[0] : null);
                    }
                    
                    // آپدیت کانفیگ جهانی برنامه
                    onConfigUpdate(newConfig);
                    notify.show('موفقیت', 'دسته‌بندی با موفقیت حذف شد.', 'success');
                } else {
                    notify.show('خطا', 'خطا در ذخیره تغییرات', 'error');
                }
            } catch (e) {
                            ...
                            >
                                ...
                                {key !== 'General' && (
                                    <div className="flex items-center gap-1">
                                        ...
                                        <button onClick={(e) => { e.stopPropagation(); handleDeleteCategory(key); }} className="text-red-400 hover:bg-red-500/20 p-1.5 rounded transition" title="حذف دسته">
        ...
        <div className="flex-1 p-8 overflow-y-auto">
            ...
            <div className="glass-panel p-6 rounded-2xl border border-white/10">
                <div className="bg-black/20 rounded-xl p-4 min-h-[300px] overflow-y-auto custom-scroll space-y-3">
                    {backups.length === 0 ? (
                        <div className="text-center text-gray-500 py-10">هیچ فایل پشتیبانی یافت نشد.</div>
                    ) : (
                        backups.map((b, idx) => (

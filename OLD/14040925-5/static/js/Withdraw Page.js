...
const WithdrawPage = ({ user, serverStatus }) => {
    ...
    }, [searchTerm, partsList, selectedCategory]);
    ...
    const addToCart = (part, exactQty = null) => {
        ...
        if (remainingStock <= 0) {
            return notify.show('موجودی ناکافی', 'تمام موجودی این قطعه در لیست خروج قرار دارد.', 'error');
        }

        if (exactQty !== null) {
            qtyToAdd = exactQty;
            if (qtyToAdd > remainingStock) {
                return notify.show('خطا', `فقط ${remainingStock} عدد دیگر قابل افزودن است.`, 'warning');
            }
        }
    ...
    const updateCartQty = (id, delta) => {
        ...
        if (newQty > part.quantity) {
            return notify.show('خطا', `موجودی ناکافی. کل موجودی: ${part.quantity}`, 'error');
        }
    ...
    const handleCheckout = async () => {
        ...
        if (!projectReason.trim()) return notify.show('توجه', 'لطفاً نام پروژه یا دلیل مصرف را وارد کنید', 'error');
        
        const confirmed = await dialog.ask('تایید خروج کالا', `آیا از ثبت خروج ${cart.length} قلم کالا برای پروژه "${projectReason}" اطمینان دارید؟`, 'warning');
        ...
        if (confirmed) {
            try {
                ...
                if (ok) {
                    notify.show('موفقیت', 'قطعات با موفقیت از انبار خارج شدند.', 'success');
                    setCart([]);
                    setProjectReason("");
                    loadParts();
                } else {
                    notify.show('خطا', data.error || 'خطا در ثبت خروج', 'error');
                }
            ...
            <header className="mb-4 flex flex-col gap-4">
                ...
                <div className="flex flex-col md:flex-row gap-3">
                    <div className="relative flex-1">
                        ...
                        <input className="nexus-input w-full pr-10 pl-4 py-3 text-sm shadow-lg border-white/10 focus:border-nexus-primary/50" placeholder="جستجوی هوشمند (نام، مقدار، پکیج، آدرس...)" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} autoFocus />
                    </div>
                                ...
                                cart.map((item) => (
                                    <div key={item.id} className="bg-[#1e293b] p-3 rounded-xl border border-white/5 group relative overflow-hidden transition hover:border-orange-500/30">
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex flex-col"><span className="text-white font-bold text-sm ltr font-mono text-right">{item.val}</span><span className="text-[10px] text-gray-400">{item.package}</span></div>
                                        ...
                                        <div className="flex items-center justify-between bg-black/20 rounded-lg p-1">
                                            ...
                                            <span className="font-mono font-bold text-orange-400 text-sm">{item.qty}</span>
                                ...
                                filteredParts.map(part => {
                                    ...
                                    return (
                                        <div key={part.id} className={`glass-panel p-4 rounded-2xl border transition-all duration-200 group relative flex flex-col justify-between hover:-translate-y-1 hover:shadow-xl ${inCart ? 'border-nexus-primary/50 bg-nexus-primary/5' : 'border-white/5 hover:border-white/20'}`}>
                                            ...
                                            <div>
                                                <div className="flex justify-between items-start mb-2">
                                                    <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{part.type}</span>
                                                ...
                                                <div className="flex items-center gap-2 mb-4">
                                                    <span className="text-xs bg-white/5 text-gray-300 px-2 py-1 rounded-md">{part.package}</span>
                                                    <span className="text-[10px] text-gray-500 flex items-center gap-1"><i data-lucide="map-pin" className="w-3 h-3"></i> {part.storage_location}</span>
                                                </div>
                                            </div>
                                            <div className="mt-auto pt-3 border-t border-white/5 flex gap-2">
                                                <button onClick={() => addToCart(part, 1)} disabled={part.quantity <= 0} className="flex-1 bg-white/5 hover:bg-nexus-primary hover:text-white text-gray-300 py-2 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed group-hover:bg-white/10"><i data-lucide="plus" className="w-4 h-4"></i>افزودن</button>

...
def init_db():
    try:
        with get_db_connection() as conn:
            ...
            # --- Schema Migrations ---
            # افزودن ستون‌های جدید به صورت امن در صورت عدم وجود
            try: conn.execute("ALTER TABLE resistors ADD COLUMN last_modified_by TEXT")
            except: pass
            try: conn.execute("ALTER TABLE resistors ADD COLUMN storage_location TEXT")
            except: pass
            
            # افزودن ستون تکنولوژی (tech) برای ذخیره نوع دقیق (Thin Film, X7R, etc.)
            try: conn.execute("ALTER TABLE resistors ADD COLUMN tech TEXT")
            except: pass 
...
def save_part():
    try:
        ...
        # مهندسی: نگاشت فیلدهای ورودی به ستون‌های دیتابیس
        # type -> Category (Resistor, Capacitor...)
        # tech -> Technology (Thin Film, X7R...)
        # watt -> Rating (Wattage or Voltage)
        ...
        payload = {
            ...
            "storage_location": d.get("location", ""),
            "tech": d.get("tech", "") # ستون جدید تکنولوژی
        }
        ...
        with get_db_connection() as conn:
            if part_id:
                ...
                conn.execute("""UPDATE resistors SET val=?, watt=?, tolerance=?, package=?, type=?, buy_date=?, quantity=?, toman_price=?, reason=?, min_quantity=?, vendor_name=?, last_modified_by=?, storage_location=?, tech=? WHERE id=?""", (*payload.values(), part_id))
                rid = part_id
            else:
                cur = conn.execute("""INSERT INTO resistors (val, watt, tolerance, package, type, buy_date, quantity, toman_price, reason, min_quantity, vendor_name, last_modified_by, storage_location, tech) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", tuple(payload.values()))
                rid = cur.lastrowid

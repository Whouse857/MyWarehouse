...
def get_inventory_stats() -> Response:
    try:
        # دریافت قیمت دلار (روزانه) - ایمن
        daily_usd_price = fetch_daily_usd_price()
        
        with get_db_connection() as conn:
            # Added watt, tolerance, tech, vendor_name to the select query
            ...
            # متغیر جدید برای محاسبه ارزش انبار به روش درخواستی شما
            ...
            for row in rows:
                q = row['quantity'] or 0
                p = row['toman_price'] or 0.0 # قیمت خرید تومانی
                u = row['usd_rate'] or 0.0    # نرخ دلار در زمان خرید
                min_q = row['min_quantity'] or 0
                cat = row['type'] or 'Uncategorized'
                
                total_quantity += q
                
                # --- منطق جدید محاسبه ارزش انبار ---
                # اگر نرخ دلار موقع خرید و نرخ دلار امروز موجود باشد:
                # ارزش = (قیمت خرید تومان / نرخ دلار خرید) * تعداد * نرخ دلار امروز
                if u > 0 and daily_usd_price > 0:
                    current_part_value_toman = (p / u) * q * daily_usd_price
                else:
                    # در غیر این صورت (برای جلوگیری از صفر شدن)، همان ارزش دفتری (قیمت خرید * تعداد) لحاظ می‌شود
                    current_part_value_toman = p * q
                
                total_value_toman_calculated += current_part_value_toman
                
                # محاسبه ارزش دلاری لحظه‌ای
                if daily_usd_price > 0:
                    total_value_usd_live += current_part_value_toman / daily_usd_price
                
                # تشخیص کسری موجودی
                if q <= min_q:
                    # Parse purchase_links safely
                    links = []
                    try:
                    ...
                    shortages.append({
                        ...
                        # Adding extra details for display
                        "type": row['type'],
                        "watt": row['watt'],
                ...
                # دسته‌بندی (بر اساس ارزش محاسبه شده جدید)
            ...
            return jsonify({
                ...
                "total_value_toman": int(total_value_toman_calculated), # ارسال ارزش محاسبه شده جدید
                "total_value_usd_live": int(total_value_usd_live),

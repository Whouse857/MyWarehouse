...
// نسخه نهایی: نمایش لینک‌های خرید به صورت دکمه
...
// کامپوننت داخلی برای نمایش لینک‌ها
...
const InventoryPage = () => {
    ...
    const handlePrintShortages = () => {
        ...
        let htmlContent = `
            ...
            <body>
                <h2>لیست کسری و سفارش خرید قطعات</h2>
                <p>تاریخ: ${today}</p>
                <table>
                    <thead>
                        <tr>
                            <th style="width: 5%">#</th>
                            <th style="width: 30%">قطعه</th>
                            <th style="width: 15%">پکیج</th>
                            <th style="width: 10%">موجودی</th>
                            <th style="width: 10%">حداقل</th>
                            <th style="width: 30%">لینک خرید (اولین مورد)</th>
                        </tr>
                    </thead>
                    <tbody>
        `;
        ...
        stats.shortages.forEach((item, index) => {
            const firstLink = (item.links && item.links.length > 0) ? item.links[0] : '-';
            htmlContent += `
                <tr>
                    <td>${index + 1}</td>
                    ...
                    <td>${item.qty}</td>
                    <td>${item.min}</td>
                    <td class="text-right" style="font-size:10px; word-break:break-all;">${firstLink}</td>
                </tr>
            `;
        });
            ...
            <header className="mb-8 flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-400 text-xs mt-1 font-medium">
                        نرخ دلار امروز: <span className="text-emerald-400 font-mono ltr">{stats.live_usd_price ? formatDecimal(stats.live_usd_price) : '---'} T</span>
                    </p>
                </div>
                        ...
                        <div className="glass-panel rounded-2xl border border-rose-500/20 overflow-hidden">
                            <div className="p-4 bg-rose-500/10 border-b border-rose-500/20 flex justify-between items-center">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button onClick={handlePrintShortages} className="px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg transition flex items-center gap-2 shadow-lg"><i data-lucide="printer" className="w-4 h-4"></i>پرینت</button>
                                </div>
                            </div>
                            <div className="overflow-x-auto">
                                        ...
                                        {stats.shortages.map((item, idx) => {
                                            ...
                                            return (
                                                <tr key={idx} className="hover:bg-white/5 transition group">
                                                    ...
                                                    <td className="px-4 py-3 text-xs text-gray-400"><div className="flex flex-wrap gap-1">{item.watt && <span className="bg-white/5 px-1 rounded">{item.watt}</span>}{item.tolerance && <span className="bg-white/5 px-1 rounded">{item.tolerance}</span>}{item.tech && <span className="text-gray-500 block w-full truncate max-w-[100px]">{item.tech}</span>}</div></td>

...
const BackupPage = () => {
    ...
    const handleCreateManualBackup = async () => {
        try { 
            ...
            if (ok) { 
                notify.show('موفقیت', `نسخه ${data.filename} ایجاد شد.`, 'success'); 
                loadBackups(); 
            } else {
                notify.show('خطا', data.error || "خطا در ایجاد بک‌آپ", 'error');
            }
        } catch (e) {
                    ...
                    {backups.length === 0 ? (
                        ...
                        backups.map((b, idx) => (
                            <div key={idx} className="flex justify-between items-center bg-white/5 p-4 rounded-xl hover:bg-white/10 transition border border-white/5">
                                <div className="flex items-center gap-4"><div className="w-10 h-10 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-400"><i data-lucide="database" className="w-5 h-5"></i></div><div className="flex flex-col"><span className="text-white font-bold text-sm ltr font-mono text-left">{b.name}</span><div className="flex gap-3 text-xs text-gray-400 mt-1"><span>{b.date}</span><span>{b.size} KB</span></div></div></div>

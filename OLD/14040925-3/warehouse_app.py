...
def init_db():
    try:
        with get_db_connection() as conn:
            ...
            # اضافه کردن ستون‌های جدید به لاگ برای دیتابیس‌های قدیمی
...
def save_part() -> tuple:
    try:
        ...
        with get_db_connection() as conn:
            ...
            # ذخیره کامل جزئیات در لاگ
            conn.execute("""
                INSERT INTO purchase_log (
                    resistor_id, val, quantity_added, unit_price, vendor_name, purchase_date, reason, operation_type, username,
                    watt, tolerance, package, type, storage_location, tech, usd_rate
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            for item in items:
                ...
                # دریافت اطلاعات کامل برای ثبت در لاگ
    ...
    with get_db_connection() as conn:
        try:
            ...
            if part: 
                conn.execute("""
                    INSERT INTO purchase_log (
                        resistor_id, val, quantity_added, operation_type, reason,
                        watt, tolerance, package, type, storage_location, tech
